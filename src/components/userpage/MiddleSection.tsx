...
const MiddleSection:React.FC<MiddleSectionProps> = ({userData,selectedCategory,setSelectedCategory}) => {
  ...
  const getFOTD = async () => {
    ...
    try {
      ...
      const timeDiff = lastFotdDate ? currentDate.getTime() - lastFotdDate?.getTime() : 86400000
      ...
      if (!usersSnapshot.empty) {
        ...
        while (retry < maxRetry){
          ...
          const isUserInMyFriends = messagingStateValue.myFriends.some((friend) => friend.id === randomUser.id);
